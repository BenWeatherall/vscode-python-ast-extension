...
- Fix plan exists in `_bugs/{bug_name}/plans/master/` directory (created by plan-bug-fix command)
- Plan contains:
...
1. **Read Fix Plan**: Read the plan documents from `_bugs/{bug_name}/plans/master/` to understand:
...
7. **Create Individual Task Documents**: For each decomposed task:
   - Create a task document in `_bugs/{bug_name}/tasks/` directory
...
8. **Ensure Tasks Directory Exists**: Create the `_bugs/{bug_name}/tasks/` directory if it doesn't exist.
