...
Review the bug report or error description provided by the user and then generate a set of investigation documents. Reference `@docs/AI_CONTEXT/` for existing project details. The documentation you create should be stored in `_bugs/{bug_name}/investigation/`. Remember that any analysis should adhere to our `@.cursor/rules/development_practices.mdc`.
...
3. **Create Investigation Documents**: Generate documents in `_bugs/{bug_name}/investigation/`:
