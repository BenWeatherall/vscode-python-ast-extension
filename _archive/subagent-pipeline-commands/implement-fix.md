...
- Bug artifacts must exist in `_bugs/{bug_name}/` directory:
...
- [ ] Step 1.3: Read investigation, research, plan, and task documents from `_bugs/{bug_name}/`
- [ ] Step 1.4: Identified affected component(s)

**Phase 2: Planning**
- [ ] Step 2.1: Created implementation plan per task (saved to `_bugs/{bug_name}/plans/tasks/`)
...
Determine which service(s) or component(s) are affected (Python service, webview UI, extension host, utilities) based on `affected-components.md` and the fix plan.
...
For each task in `_bugs/{bug_name}/tasks/`, create an implementation plan that includes:
...
Save plans to `_bugs/{bug_name}/plans/tasks/` with file names matching task files (e.g., `01-add-regression-test.md`).
...
- **Read from _bugs/**: This command consumes `_bugs/{bug_name}/`, not `_features/`
