...
- Task plan exists in `_bugs/{bug_name}/plans/tasks/{task_file}.md` (created by architect)
...
- [ ] Step 3.3: Archived task and plan files to `_archive/fix-{bug_name}/`
...
1. Read the task from `_bugs/{bug_name}/tasks/{task_file}.md`
2. Read the plan from `_bugs/{bug_name}/plans/tasks/{task_file}.md`
3. If the plan seems incomplete, request the architect to update it before proceeding

### Step 1.4: Read Scratchpad

Read `@.cursor/scratchpad.md` for bug name, pipeline state, and prior task completions.
...
- Validation steps from `_bugs/{bug_name}/plans/master/validation.md`
...
Complete validation steps from `_bugs/{bug_name}/plans/master/validation.md` to confirm fix and prevent regression.
...
- Brief description of the bug fix
  - Plan: [task-name](_archive/fix-{bug_name}/plans/tasks/{task_file})
  - Task: [task-name](_archive/fix-{bug_name}/tasks/{task_file})
```
...
```
mkdir -p _archive/fix-{bug_name}/tasks
mkdir -p _archive/fix-{bug_name}/plans/tasks
mv _bugs/{bug_name}/tasks/{task_file} _archive/fix-{bug_name}/tasks/
mv _bugs/{bug_name}/plans/tasks/{task_file} _archive/fix-{bug_name}/plans/tasks/
```
...
- Archive path uses `fix-` prefix: `_archive/fix-{bug_name}/`
