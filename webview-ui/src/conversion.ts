/** Utilities for snake_case ↔ camelCase conversion between Python and TypeScript. */
...
export function pythonNodeDataToTs(data: PythonNodeData): NodeData {
...
export function pythonNodeToTs(node: PythonReteNode): ReteNode {
  return {
    id: node.id,
    label: node.label,
    inputs: node.inputs as Record<string, import("./types").Socket>,
    outputs: node.outputs as Record<string, import("./types").Socket>,
...
export function pythonConnectionToTs(conn: PythonReteConnection): ReteConnection {
  return {
    source: conn.source,
